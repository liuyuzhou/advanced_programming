...
class UrlPathFinder(importlib.abc.PathEntryFinder):
    ...
    def find_loader(self, full_name):
        log.debug(f'find_loader: {full_name}')
        parts = full_name.split('.')
        base_name = parts[-1]
        # Check link cache
        ...
        if base_name in self._links:
            log.debug(f'find_loader: trying package {full_name}')
            full_url = self._base_url + '/' + base_name
            # Attempt to load the package (which accesses __init__.py)
            loader = UrlPackageLoader(full_url)
            try:
                loader.load_module(full_name)
                log.debug(f'find_loader: package {full_name} loaded')
            except ImportError as e:
                log.debug(f'find_loader: {full_name} is a namespace package')
                loader = None
            return loader, [full_url]

        # A normal module
        file_name = base_name + '.py'
        if file_name in self._links:
            log.debug(f'find_loader: module {full_name} found')
            return self._loader, []
        else:
            log.debug(f'find_loader: module {full_name} not found')
            return None, []
